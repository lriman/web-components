...
export const validators = [
  ...
  function type({ type: typeProp }) {
    ...
    if (typeProp !== NT.SUCCESS && typeProp !== NT.ERROR && typeProp !== NT.NORMAL && typeProp !== NT.WARNING) {
      throw new Error(ERROR.TYPE_NOT_EXISTENT);
    }
  },

  function container({ container: containerProp }) {
    if (isNull(containerProp)) {
      throw new Error(ERROR.CONTAINER_REQUIRED);
    }

    if (!isString(containerProp)) {
