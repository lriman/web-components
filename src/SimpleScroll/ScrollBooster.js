...
export default class ScrollBooster {
  ...
  constructor(options = {}) {
    ...
    if (!this.props.viewport || !(this.props.viewport instanceof Element)) {  // eslint-disable-line
  ...
  updateMetrics(node) {
    ...
    this.content = {
      width: getFullWidth(node),
      height: getFullHeight(node),
    };
    this.edgeX = {
      from: Math.min(-node?.width + this.viewport.width, 0),
      to: 0,
    };
    this.edgeY = {
      from: Math.min(-node?.height + this.viewport.height, 0),
      to: 0,
    };
  ...
  isMoving() {
    return (
      this.isDragging || this.isScrolling || Math.abs(this.velocity.x) >= 0.01 || Math.abs(this.velocity.y) >= 0.01
    );
  }
   ...
   */
  getDragAngle(x, y) {  // eslint-disable-line
   ...
   */
  getDragDirection(angle, tolerance) { // eslint-disable-line
  ...
  handleEvents() {
    ...
    this.events.pointerdown = event => {
      ...
      if (Math.abs(this.position.x - this.dragStartPosition.x) > 50) this.onLoadData();
    ...
    this.events.pointermove = event => {
      ...
      if (
        event.cancelable &&
        (this.props.lockScrollOnDragDirection === 'all' || this.props.lockScrollOnDragDirection === dragDirection)
      ) {
        event.preventDefault();
      }
      setDragPosition(event);
    ...
    this.events.wheel = event => {
      ...
      wheelTimer = setTimeout(() => (this.isScrolling = false), 80);  // eslint-disable-line

      // get (trackpad) scrollDirection and prevent default events
      if (
        this.props.preventDefaultOnEmulateScroll &&
        this.getDragDirection(this.getDragAngle(-event.deltaX, -event.deltaY), this.props.dragDirectionTolerance) ===
          this.props.preventDefaultOnEmulateScroll
      ) {
        event.preventDefault();
      }
  ...
  enableOnLoadData(eventHandler) {
    this.onLoadData = eventHandler;
  }
