...
import {
  getChar,
  getCaretPosition,
  formatNumberDecimalValue,
  deformatNumberDecimalValue,
  getNumberDisplayedValue,
  getNumberPureValue,
} from './_utils';

export default class InputNumberDecimal extends PureComponent {
  static REFS = REFS;

  static inputType = TYPES.NUMBER_DECIMAL;
  ...
  static propTypes = {
    ...
    frictionLength: PropTypes.number,
  ...
  constructor(props) {
    ...
    const { value, maxLength, units, frictionLength, isFormatted, isNegative } = this.props;
  ...
  static getDerivedStateFromProps(props, state) {
    if (!state.isInitialized || props.value !== state.value) {
      const formattedValue = formatNumberDecimalValue(
        props.value,
        props.isNegative,
        props.isFormatted,
        props.maxLength,
        props.frictionLength,
        !state.isFocused,
      );
      const value = deformatNumberDecimalValue(formattedValue);
  ...
  handleFocus = () => {
    const { id, onFocus, isNegative, isFormatted, maxLength, frictionLength } = this.props;
    const { value } = this.state;

    const formattedValue = formatNumberDecimalValue(value, isNegative, isFormatted, maxLength, frictionLength, false);
  ...
  handleBlur = () => {
    const { id, onBlur, isNegative, isFormatted, maxLength, frictionLength } = this.props;
    const { value } = this.state;

    const formattedValue = formatNumberDecimalValue(value, isNegative, isFormatted, maxLength, frictionLength, true);
  ...
  handleChange = e => {
    ...
    const { id, units, onChange, isNegative, isFormatted, maxLength, frictionLength } = this.props;
    ...
    const deformattedValue = deformatNumberDecimalValue(formattedValue);
  ...
  handleKeyPress = e => {
    ...
    if (
      ...
      ((char !== null && (char < '0' || char > '9') && !['-', '.', ','].includes(char)) ||
        ...
        (['.', ','].includes(char) && e.target.value.indexOf(',') > -1))
    ) {
      e.preventDefault();
    }
  };
      ...
      <ColorContext.Consumer>
        {({ colorType = 'normal' }) => (
          <StyledInput {...addDataAttributes({ component: COMPONENTS.GENERAL })} sWidth={width} isWithLabel={!!label}>
            <StyledInputText
              {...addDataAttributes({ component: TYPES.NUMBER_DECIMAL })}
              as="input"
              ref={this.inputNode}
              id={id}
              sSize={size}
